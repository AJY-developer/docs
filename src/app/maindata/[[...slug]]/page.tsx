...
export default function MainData({ params }: { params: { slug: string[] } }) {
  ...
  function getData(slug: string[]) {
    if (params.slug.length == 1) {
      ...
      setLoading(true);
      fetch(`/api/contentManageApi?topic=${slug[0]}&urlname=${slug[1]}`, {
        cache: "no-store",
      })
        .then((res) => res.json())
        .then((content) => {
  ...
  function deleteRecord() {
    if (params.slug.length == 1) {
      ...
      fetch(
        `/api/contentManageApi?topic=${params.slug[0]}&urlname=${params.slug[1]}`,
        {
          method: "DELETE",
        }
      )
    ...
    return (
      <>
        <div className=" relative pb-20">
          ...
          {isadmin ? (
            <Adminaccess params={params} deleteRecord={deleteRecord} />
          ) : (
            ""
          )}
